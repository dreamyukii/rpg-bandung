...
func _process(delta):
	if current_enemy1_health == -1 && current_enemy2_health == -1 && current_enemy3_health == -1:
		SceneTransition.Transit("res://scene/Scene8.tscn")
...
func enemy_turn():
	if current_enemy1_health > 0:
		display_text("Kamu diserang Enemy Commander1 sebanyak 15 dmg")
		slash2.play()
		...
		set_health(current_player_health, State.max_health, $Player/ProgressBar)
			...
			display_text("Enemy Commander1 mati")
			emit_signal("textbox_closed")
	elif current_enemy1_health == -1:
		display_text("Enemy Commander1 sudah mati, tidak dapat menyerang")
...
func enemy3_turn():
	if current_enemy3_health > 0:
		display_text("Kamu diserang Enemy Commander2 sebanyak 15 dmg")
		$AnimationPlayer.play("shake")
		slash2.play()
		await self.textbox_closed
		...
		set_health(current_player_health, State.max_health, $Player/ProgressBar)
			...
			display_text("Enemy Commander2 mati")
	elif current_enemy3_health == -1:
		display_text("Enemy Commander2 sudah mati, tidak dapat menyerang")
